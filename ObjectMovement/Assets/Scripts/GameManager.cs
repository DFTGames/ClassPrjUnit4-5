...
public class GameManager : MonoBehaviour {
	...
	void Start() {
        Application.runInBackground = true;//cosi' mi si muove nel altro client anche se non ho la finestra attivata
	...
	private void SpawnLocalPlayer(int numModel, int numMaterial) {
		...
		userVariables.Add(new SFSUserVariable("mat", numMaterial));
		sfs.Send(new SetUserVariablesRequest(userVariables)); //ninfea non sa come..ma si accorge del users..
	}
	...
	private void SpawnRemotePlayer(SFSUser user, int numModel, int numMaterial, Vector3 pos, Quaternion rot) {
		...
		if (remotePlayers.ContainsKey(user) && remotePlayers[user] != null) {//e' stato fatto perche durante il gioco puoi camvbiare modello ecc..
